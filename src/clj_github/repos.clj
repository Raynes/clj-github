...
(defn create-repo
  ...
  [name & {:keys [public homepage description] :or {public true}}]
  {:pre [(not (nil? name))]}
...
(defn show-watchers
  ...
  [user repo & {full-profile? :full-profile? :or {full-profile? false}}]
  (make-request ["repos/show" user repo "watchers"] :data (when full-profile? {"full" 1}) :sift :watchers))
